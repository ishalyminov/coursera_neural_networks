...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  [inputs_number, samples_number] = size(data.inputs);
  [hidden_dim, input_dim] = size(model.input_to_hid);
  error_deriv = class_prob - data.targets;
  hid_to_class_gradient = hid_output * error_deriv' ./ samples_number;

  back_propagated_deriv = (model.hid_to_class' * error_deriv) .* hid_output .* (1 - hid_output) ./ samples_number;
  input_to_hid_gradient = zeros(hidden_dim, inputs_number);

  for w = 1:inputs_number
    input_to_hid_gradient(:, w) = input_to_hid_gradient(:, w) + (data.inputs(w, :) * back_propagated_deriv')';
  end

  %% compute derivative of weight decay loss
  wd_deriv = wd_coefficient;

  ret.input_to_hid = (model.input_to_hid .* wd_deriv) + input_to_hid_gradient;
  ret.hid_to_class = (model.hid_to_class .* wd_deriv) + hid_to_class_gradient';
end
